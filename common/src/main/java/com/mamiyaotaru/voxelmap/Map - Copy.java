...
//        this.drawUHCBorderOnMinimap(matrixStack, x, y);
//
...
//    }
//
//    private void drawUHCBorderOnMinimap(Matrix4fStack matrixStack, int centerX, int centerY) {
//        BorderStatus status = net.kimkung.dmsuhc.client.event.border.UHCBorder.getStatus();
//
//        double playerX = GameVariableAccessShim.xCoordDouble();
//        double playerZ = GameVariableAccessShim.zCoordDouble();
//
//        float pixelsPerBlock = (float) (1.0 / this.zoomScaleAdjusted);
//
//        float bMinX = (float) ((net.kimkung.dmsuhc.client.event.border.UHCBorder.getMinX(0f) - playerX) * pixelsPerBlock) + centerX;
//        float bMaxX = (float) ((net.kimkung.dmsuhc.client.event.border.UHCBorder.getMaxX(0f) - playerX) * pixelsPerBlock) + centerX;
//        float bMinZ = (float) ((net.kimkung.dmsuhc.client.event.border.UHCBorder.getMinZ(0f) - playerZ) * pixelsPerBlock) + centerY;
//        float bMaxZ = (float) ((net.kimkung.dmsuhc.client.event.border.UHCBorder.getMaxZ(0f) - playerZ) * pixelsPerBlock) + centerY;
//
//        float r = 32.0f;
//        float clipX1 = centerX - r, clipX2 = centerX + r;
//        float clipZ1 = centerY - r, clipZ2 = centerY + r;
//
//        if (bMaxX < clipX1 || bMinX > clipX2 || bMaxZ < clipZ1 || bMinZ > clipZ2) return;
//
...
//
//        float lw = 1.0f;
//        float depth = MAP_OVERLAY_DEPTH + 1;
//
//        // ใช้ resourceSquareMapStencil เป็น dummy texture สีขาว
//        // แล้วใส่ color tint ทำให้ได้ filled rect
//        RenderType lineType = VoxelMapRenderTypes.GUI_TEXTURED_LEQUAL_DEPTH_TEST.apply(resourceSquareMapStencil);
//        VertexConsumer buf = renderBufferSource.getBuffer(lineType);
//
//        // top edge (minZ)
//        if (bMinZ >= clipZ1 && bMinZ <= clipZ2) {
//            float x1 = Math.max(bMinX, clipX1), x2 = Math.min(bMaxX, clipX2);
//            RenderUtils.drawTexturedModalRect(matrixStack, buf, x1, bMinZ - lw, depth, x2 - x1, lw * 2, borderColor);
//        }
//        // bottom edge (maxZ)
//        if (bMaxZ >= clipZ1 && bMaxZ <= clipZ2) {
//            float x1 = Math.max(bMinX, clipX1), x2 = Math.min(bMaxX, clipX2);
//            RenderUtils.drawTexturedModalRect(matrixStack, buf, x1, bMaxZ - lw, depth, x2 - x1, lw * 2, borderColor);
//        }
//        // left edge (minX)
//        if (bMinX >= clipX1 && bMinX <= clipX2) {
//            float z1 = Math.max(bMinZ, clipZ1), z2 = Math.min(bMaxZ, clipZ2);
//            RenderUtils.drawTexturedModalRect(matrixStack, buf, bMinX - lw, z1, depth, lw * 2, z2 - z1, borderColor);
//        }
//        // right edge (maxX)
//        if (bMaxX >= clipX1 && bMaxX <= clipX2) {
//            float z1 = Math.max(bMinZ, clipZ1), z2 = Math.min(bMaxZ, clipZ2);
//            RenderUtils.drawTexturedModalRect(matrixStack, buf, bMaxX - lw, z1, depth, lw * 2, z2 - z1, borderColor);
//        }
//
//        renderBufferSource.endBatch();
//    }
